...
function setup() {
  ...
  fft.setInput(mic);
    ...
    var displayWidthC = window.innerWidth
|| document.documentElement.clientWidth
|| document.body.clientWidth;

var displayHeightC = window.innerHeight
|| document.documentElement.clientHeight
|| document.body.clientHeight;
    
  createCanvas(displayWidthC, displayHeightC);

  startingX = displayHeightC * 0.01;
  startingY = displayHeightC * 0.01;
  typeSpacing = 43;
  typeSize = 1;

  charSlider = createSlider(0, 25, 0, 1);
  charSlider.position(displayWidthC / 2, displayHeightC * 0.95);

  threshholdSlider = createSlider(0, 100, 0, 0.5);
  threshholdSlider.position(displayWidthC / 2, displayHeightC * 0.85);
}

/*function mousePressed() {
  if (
    mouseX > 0 &&
    mouseX < displayWidthC &&
    mouseY > 0 &&
    mouseY < displayHeightC
  ) {
    let fs = fullscreen();
    fullscreen(true);
  }
...
function drawType() {
  ...
  if (avgCounter < 46) {
    ...
    widthMap = map(40, 15, 200, typeWeight * 2 + 10, displayWidthC);
  ...
  stroke(2);
  line(0, ascenders, displayWidthC, ascenders);
  text("ascenders", displayWidthC / 2, ascenders);

  ellipse(ascenders, ascenders, 20, 20);

  unders = displayHeightC * 0.94 - weightMap / 2;
  line(0, unders, displayWidthC, unders);
  text("unders", displayWidthC / 2, unders);
  ...
  line(0, xHeight, displayWidthC, xHeight);
  text("xHeight", displayWidthC / 2, xHeight);

  marki = length * 0.10789980732 + weightMap;
  line(0, marki, displayWidthC, marki);
  text("marki", displayWidthC / 2, marki);

  mark1 = length * 0.36223506743 + weightMap;
  line(0, mark1, displayWidthC, mark1);
  text("mark1", displayWidthC / 2, mark1);

  mark2 = length * 0.53179190751;
  line(0, mark2, displayWidthC, mark2);

  text("mark2", displayWidthC / 2, mark2);

  mark3 = length * 0.70134874759;
  line(0, mark2, displayWidthC, mark2);

  text("mark3", displayWidthC / 2, mark3);

  baseLine = length * 0.82851637764 + weightMap / 2;
  line(0, baseLine, displayWidthC, baseLine);

  text("baseline", displayWidthC / 2, baseLine);

  //typeWidth = widthSlider.value() - typeWeight;
  typeWidth = widthMap - typeWeight;

  // startingX = displayWidthC/2 - typeWeight/2;
  // startingY = displayHeightC/2 - length*0.5;
