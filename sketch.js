...
function drawType() {
  ...
  radiusMap = map(calculateAverage(0, 15), radiusMapMin, radiusMapMax, 40, -50);
  if (radiusMap < 0) radiusMap= 0;
  ...
  calc2 = calc2*calc2;
  weightMap = map(calc2, 0, 3000, 20, 200);
  ...
  noStroke();
  let c1 = map(radiusMap, 0, 40,255,0);
  let c2 = map(weightMap, 20, 200, 0,255);
  fill(c2, c1, typeColorB);
  /*if (typeColorR != 0) {
    typeColorR--;
  }
  if (typeColorG != 0) {
